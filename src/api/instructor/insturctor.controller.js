...
/* signup */
  async signup (req, res) {
    try {
      const { fullName, email, password, isInstructor } = req.body

      const encriptedPassord = await bcrypt.hash(password, 11)
      isInstructor(true)
      const user = await User.create({ fullName:fullName, email:email, password:encriptedPassord, isInstructor:isInstructor })
